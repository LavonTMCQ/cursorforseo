...
const contentAnalysisTool = createTool({
  id: "content-analysis",
  ...
  execute: async ({ context }) => {
    const { content, targetKeyword, metaTitle, metaDescription } = context;
    const analysis = await analyzeContent(content, targetKeyword, metaTitle, metaDescription);
    return analysis;
  }
});
async function analyzeContent(content, targetKeyword, metaTitle, metaDescription) {
  ...
  return {
    ...
    contentMetrics: {
      wordCount,
      readabilityScore: Math.max(0, 100 - wordCount / 50),
...
export { contentAnalysisTool };
//# sourceMappingURL=26362c2d-1a72-4ffa-bd43-e866e680312f.mjs.map
