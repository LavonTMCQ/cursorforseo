...
const projectDataTool = createTool({
  id: "project-data",
    ...
    try {
      ...
      switch (action) {
        ...
        case "get_keywords":
          ...
          break;
        case "get_rankings":
          const keywordFilter = projectId ? { keyword: { projectId } } : {};
          ...
          summary.averageRanking = data.length > 0 ? data.reduce((sum, r) => sum + r.position, 0) / data.length : 0;
...
function calculateAverageRanking(keywords) {
...
function generateProjectInsights(projects) {
  const insights = [];
...
function generateSingleProjectInsights(project) {
  const insights = [];
  if (!project) {
  ...
  if (project.keywords.length > 0) {
    ...
    const topRankingKeywords = project.keywords.filter((k) => k.rankings?.[0]?.position <= 10).length;
...
function generateKeywordInsights(keywords) {
  const insights = [];
...
function generateRankingInsights(rankings) {
  const insights = [];
...
function generateAuditInsights(audits) {
  const insights = [];
  if (audits.length === 0) {
...
export { projectDataTool };
//# sourceMappingURL=5e217580-e44c-4dca-8ee3-4a3078d5fae1.mjs.map
